...
public class BalanceHistoryRepoImpl implements BalanceHistoryRepo {
	...
	@Override
	public void create(BalanceHistoryForm form) {
		jdbcClient.sql("Insert Into BALANCE_HISTORY values (?,?,?,?,?,?,?)")
		.param(new SimplePropertySqlParameterSource(form))
		.update();
	...
	public List<UserBalanceInfo> search(UserBalanceSearch search) {
		...
		var sql = new StringBuffer("""
				select a.level , b.account_id , a.name , b.trx_id , b.issue_at ,
				t.trx_type , t.ledger , t.particular , b.before_amount , b.trx_amount
				from BALANCE_HISTORY b join on ACCOUNT a b.account_id = a.login_id join 
				TRX_BASE t on b.trx_id = t.id  WHERE 1 = 1
				""");

...
drop index if exists user_and_trx_type;
...
create table BALANCE_HISTORY(
  ...
  partucular varchar (255) not null,
  constraint balance_history_to_trx foreign key (trx_id) references TRX_BASE(id),
  constraint balance history_to_account foreign key (account _id) references ACCOUNT(login_id)
);

drop table if exists 	LIMIT_SETTING;
create table LIMIT__SETTING(
